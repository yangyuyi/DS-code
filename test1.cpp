...
public:
    ...
    virtual bool get_del(int pos, T &value) = 0;

    virtual bool isEmpty() = 0;

    void work(int n, int m, int s);
};

template<class T>
void List<T>::work(const int n, const int m, const int s) {
    int pos = s - 1;
    T value;
    cout << "The Josephus list is ";
    for (int i = 0; i < n; i++) {
...
public:
    ...
    bool get_del(int pos, T &value) override;
...
int main() {
    ...
    cin >> n;
    while (n <= 0) {
        cout << "number n is illegal. n must be a positive number." << endl;
        cout << "number n=";
        cin >> n;
    }
    ...
    cin >> m;
    while (m <= 0) {
        cout << "number m is illegal. m must be a positive number." << endl;
        cout << "number m=";
        cin >> m;
    }
